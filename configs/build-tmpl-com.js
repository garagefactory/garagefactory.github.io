  ...
  string: {
    ...
    EJSpreviewFilter(data) {
      return (s => {
        if (typeof s === 'string') {
          let addCode = ''
          const subd = (data.subdir_css || '') + '/'
          data.styles.push(path.basename(data.file, '.ejs'))
          for (const css of data.styles) {
            addCode += `<link rel="stylesheet" href="${subd + css}.css"></link>`
          }
          s = s
            .repSentence(data.testcase)
            .replace('[[[__preview_styleset__]]]', addCode)
            .replace(/<{(.+)}>/g, (_, y) => {
              if (y[0] === ('$')) {
                return y
              }
...
module.exports = {
  ...
  setupCMS(ign) {
    ...
    // copy assets
    // if (this.cms[cname].throw_assets) {
    //   let dist
    //   const CnR = (pp) => pp.replace('[cname]', cname)
    //   if (typeof this.cms[cname].throw_assets === 'string') {
    //     dist = CnR(this.Dist) + '/' + this.cms[cname].throw_assets
    //   }
    //   if (typeof this.cms[cname].throw_assets === 'boolean') {
  ...
  genFilePath(o, data) {
    makeDir.sync(o.dp)
    fs.writeFile(path.format({ dir: o.dp, name: o.basename, ext: '.' + o.format }), data, () => { })
  },
  setDployPoint(o) {
    ...
    if (o.path.indexOf(o.cn + '/') >= 0) {
      ...
      if (settype === 'boolean' && o.subdir) {
        o.dp += path.dirname(o.path.substr(o.path.lastIndexOf(`/${o.cn}/`) + o.cn.length + 1))
      }
    }
    return o.dp
  },
      ...
      (err, code) => {
        try {
          (async set => {
            ...
            this.genFilePath({ dp: 'preview', basename: 'index', format: 'html' }, code.EJSpreviewFilter(set))
          })({ subdir_css: this.cms.subdir_css, cms: 'github', ...this.cms.preview })
        } catch (_e) {
  ...
  xPass(f, lv = false) {
    ...
    //       path: f,
    //       dp: this.Dist.replace('[cname]', cn),
    //       cn,
    //       subdir: this.cms[cn].subdir_html
    //     })
    //     makeDir.sync(dp)
    //     fs.copyFileSync(f, path.resolve(dp, this.ext(f, '/')).replace('[cname]', cn))
    //     if (lv && typeof this.cms[cn].preview === 'object') this.xPreview(cn, fs.readFileSync(f, 'utf-8'))
    //   }
    // }
  },
  xEJS(f, lv = false) {
    ...
    const filePageData = (f) => {
      // for (const cn of this.cms_en) {
      //   if (this.match(f, cn)) {
      //     if (lv && typeof this.cms[cn].preview === 'object') this.cms[cn].preview.file = this.ext(f, '/')
      pd.script = (fs.existsSync(f + '.js')) ? require(require.resolve(f) + '.js') : {}
      pd.pagedata = {
        minifyJs: UglifyJS,
  ...
  xCSS(f) {
    const pp = sass.renderSync({ file: path.resolve(f), outputStyle: "compressed" }).css
    const posted = postcss([autoprefixer({ remove: false }), postcssGapProperties]).process(pp, { from: undefined }).css
